export const PAIRS = [
  {
    id: 1,
    nature: {
      label: "Aile de papillon",
      image: "/images/cards/vivant/papillon.png",
    },
    application: {
      label: "Panneaux solaires",
      image: "/images/cards/application/Panneau solaire.jpeg",
    },
    explication: "Les écailles nanométriques de l'aile du papillon morpho régulent sa température en absorbant ou réfléchissant les rayons infrarouges. Ce mécanisme inspire des panneaux photovoltaïques capables d'éviter la surchauffe, améliorant ainsi leur rendement.",
    lien: "Régulation thermique par structure nanométrique",
    indice1: "Ce papillon gère sa chaleur sans transpirer…",
    indice2: "Ses écailles absorbent ou réfléchissent la lumière selon la température.",
    indice3: "L'énergie solaire est au cœur de ce lien.",
  },
  {
    id: 2,
    nature: {
      label: "Cicatrice",
      image: "/images/cards/vivant/Cicatrice.png",
    },
    application: {
      label: "Béton autocicatrisant",
      image: "/images/cards/application/Béton cicatrisant.jpeg",
    },
    explication: "Comme la peau se répare après une blessure, le béton autocicatrisant contient des bactéries encapsulées qui, au contact de l'eau s'infiltrant dans une fissure, produisent du carbonate de calcium pour colmater la brèche automatiquement.",
    lien: "Auto-réparation autonome déclenchée par un stimulus",
    indice1: "La peau humaine se répare seule après une blessure…",
    indice2: "Des bactéries dormantes se réveillent au contact de l'eau.",
    indice3: "Ce matériau de construction se répare tout seul.",
  },
  {
    id: 3,
    nature: {
      label: "Coléoptère de Namibie",
      image: "/images/cards/vivant/Coléoptère de Namibie.png",
    },
    application: {
      label: "Filets capteurs de rosée",
      image: "/images/cards/application/Filets capteur de rose.jpeg",
    },
    explication: "Le scarabée Stenocara collecte l'eau du brouillard matinal grâce à son dos aux surfaces alternées hydrophiles et hydrophobes. Des filets géants imitant ce principe captent la rosée dans les zones arides.",
    lien: "Collecte d'eau par alternance hydrophile / hydrophobe",
    indice1: "Cet insecte survit dans l'un des déserts les plus chauds du monde…",
    indice2: "Son dos alterne zones qui attirent et zones qui repoussent l'eau.",
    indice3: "La collecte d'eau dans des zones arides est la clé.",
  },
  {
    id: 4,
    nature: {
      label: "Corail",
      image: "/images/cards/vivant/Corail.png",
    },
    application: {
      label: "Ciment neutre en carbone",
      image: "/images/cards/application/ciment.jpg",
    },
    explication: "Les polypes de corail absorbent le CO₂ et les minéraux de l'eau de mer pour sécréter du carbonate de calcium. Ce processus inspire une production de ciment qui capture le CO₂ plutôt que d'en émettre.",
    lien: "Minéralisation du CO₂ pour produire un matériau de construction",
    indice1: "Cet organisme marin construit sa propre structure dure…",
    indice2: "Il absorbe le CO₂ pour créer un matériau solide.",
    indice3: "Le secteur de la construction cherche à réduire ses émissions.",
  },
  {
    id: 5,
    nature: {
      label: "Corbeille de Vénus",
      image: "/images/cards/vivant/Corbeille de Vénus.png",
    },
    application: {
      label: "30 St Mary Axe",
      image: "/images/cards/application/30 st mary axe.jpeg",
    },
    explication: "Cette éponge de mer possède un squelette en treillis de fibres de verre disposées en diagonale — solidité remarquable. La tour londonienne The Gherkin reprend cette même morphologie spiralée.",
    lien: "Structure en treillis hélicoïdal pour une résistance maximale",
    indice1: "C'est une éponge marine au squelette en fibres de verre…",
    indice2: "Sa structure en treillis diagonal lui confère une solidité exceptionnelle.",
    indice3: "Un célèbre gratte-ciel londonien partage sa forme.",
  },
  {
    id: 6,
    nature: {
      label: "Effet Lotus",
      image: "/images/cards/vivant/Effet Lotus.png",
    },
    application: {
      label: "Surface hydrofuge",
      image: "/images/cards/application/hydrofuge.jpeg",
    },
    explication: "La feuille de lotus possède des micro-reliefs nanométriques qui empêchent les molécules d'eau d'adhérer à sa surface. L'eau perle et emporte les impuretés. Ce principe est appliqué aux façades et vitrages.",
    lien: "Super-hydrophobie par micro-relief de surface",
    indice1: "Cette plante aquatique reste toujours propre…",
    indice2: "Des micro-bosses à sa surface empêchent l'eau d'accrocher.",
    indice3: "Des façades de bâtiments utilisent ce principe autonettoyant.",
  },
  {
    id: 7,
    nature: {
      label: "Manchot",
      image: "/images/cards/vivant/Manchot.png",
    },
    application: {
      label: "District 11",
      image: "/images/cards/application/District 11.jpeg",
    },
    explication: "Les manchots se serrent en groupes denses sur la banquise, économisant jusqu'à 5°C de chaleur collective. Le District 11 à Skolkovo regroupe ses villas en cercle sur ce même principe de mutualisation thermique.",
    lien: "Mutualisation thermique par regroupement circulaire dense",
    indice1: "Cet animal survit dans des conditions de froid extrême en groupe…",
    indice2: "Leur regroupement en cercle crée un bouclier thermique naturel.",
    indice3: "Un projet architectural russe s'est inspiré de cette stratégie.",
  },
  {
    id: 8,
    nature: {
      label: "Moule",
      image: "/images/cards/vivant/Moule.png",
    },
    application: {
      label: "Colle forte bio-inspirée",
      image: "/images/cards/application/Colle fort.jpeg",
    },
    explication: "Les moules sécrètent des protéines qui leur permettent de se fixer aux rochers, même sous l'eau. Des chercheurs ont reproduit ces protéines adhésives pour créer une colle capable de lier des enzymes sur des surfaces métalliques.",
    lien: "Adhésion universelle sous l'eau par protéines bio-inspirées",
    indice1: "Cet animal de bord de mer s'accroche à tout…",
    indice2: "Ses protéines adhèrent même sous l'eau, sur n'importe quelle surface.",
    indice3: "Cette découverte vient des laboratoires du CNRS.",
  },
  {
    id: 9,
    nature: {
      label: "Zèbre",
      image: "/images/cards/vivant/Zèbre.png",
    },
    application: {
      label: "Camouflage naval",
      image: "/images/cards/application/Camouflage.jpeg",
    },
    explication: "Les rayures du zèbre créent une illusion d'optique qui perturbe la perception du mouvement. En 1917, Norman Wilkinson a appliqué ce principe en couvrant des navires britanniques de motifs contrastés pour déjouer les sous-marins ennemis.",
    lien: "Illusion d'optique par contraste pour perturber la perception",
    indice1: "Le motif de cet animal intrigue les scientifiques depuis des siècles…",
    indice2: "Ses rayures créent une illusion d'optique qui trompe les prédateurs.",
    indice3: "Cette technique fut utilisée sur des bateaux en 1917.",
  },
  {
    id: 10,
    nature: {
      label: "Os humain",
      image: "/images/cards/vivant/Os humain.png",
    },
    application: {
      label: "Tour Eiffel",
      image: "/images/cards/application/Tour effeil.jpeg",
    },
    explication: "L'os humain ne place la matière que là où la résistance est nécessaire. Gustave Eiffel s'est directement inspiré du fémur humain pour concevoir sa tour — légère, optimisée, avec la matière concentrée là où les contraintes s'exercent.",
    lien: "Optimisation structurelle — matière placée là où les forces s'exercent",
    indice1: "Ce tissu vivant optimise sa structure pour éviter la rupture…",
    indice2: "La matière n'est présente que là où la résistance est demandée.",
    indice3: "La structure la plus célèbre de Paris s'en est inspirée.",
  },
  {
    id: 11,
    nature: {
      label: "Orchidée",
      image: "/images/cards/vivant/orchidee.png",
    },
    application: {
      label: "Gardens by the Bay",
      image: "/images/cards/application/Gardens by the bay.jpeg",
    },
    explication: "Le plan directeur de Gardens by the Bay à Singapour s'inspire de la forme de l'orchidée pour créer un réseau harmonieux de structures architecturales et de technologies environnementales intégrées dans un tout organique.",
    lien: "Morphologie florale transposée à l'échelle architecturale",
    indice1: "Cette fleur tropicale est emblématique de Singapour…",
    indice2: "Sa forme inspire un plan directeur mêlant nature et technologie.",
    indice3: "Ce parc futuriste de Singapour est connu dans le monde entier.",
  },
];

export const ALL_APPLICATIONS = PAIRS.map(p => p.application);

export const RANKS = [
  { min: 0,   label: "Graine", emoji: "🌱", icon: "seed",   color: "var(--rank-seed)",   bg: "var(--brown-pale)",  desc: "Vous commencez à germer…" },
  { min: 200, label: "Pousse", emoji: "🌿", icon: "sprout", color: "var(--rank-sprout)", bg: "var(--mint-pale)",   desc: "La curiosité commence à pousser." },
  { min: 450, label: "Arbre",  emoji: "🌳", icon: "tree",   color: "var(--rank-tree)",   bg: "#DCF0DC",            desc: "Vous prenez racine dans le vivant." },
  { min: 750, label: "Forêt",  emoji: "🌲", icon: "forest", color: "var(--rank-forest)", bg: "#C8E8C0",            desc: "Vous êtes en harmonie avec la nature." },
];

export const XP_RULES = [
  { xp: 100, label: "Génial du 1er coup !",     color: "var(--rank-tree)",   bg: "#DCF0DC" },
  { xp: 60,  label: "La 2ème était la bonne !", color: "var(--mint-dark)",   bg: "var(--mint-pale)" },
  { xp: 30,  label: "De peu !",                 color: "var(--brown)",       bg: "var(--brown-pale)" },
  { xp: 10,  label: "Enfin trouvé !",           color: "var(--brown-light)", bg: "#F5EEE6" },
];



export const TIMER_TOTAL = 30;
